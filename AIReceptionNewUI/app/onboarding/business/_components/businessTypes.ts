...
export const businessTypeGroups: BusinessTypeGroup[] = [
  {
...
export const getSubTypesForCategory = (category?: string): string[] => {
  if (!category) return [];
  return businessTypeGroups.find((group) => group.category === category)?.subTypes ?? [];
};

const previewBulletsByCategory: Record<string, string[]> = {
  "Healthcare & Wellness": [
...
export const getPreviewBullets = (category?: string): string[] => {
  if (!category) {
    return [
